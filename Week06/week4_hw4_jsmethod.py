...
week4_hw4_jsmethod=Flask(
    __name__,
    static_folder="static", # 靜態檔案的資料夾名稱
    static_url_path="/" # 靜態檔案對應的網址名稱
)

week4_hw4_jsmethod.secret_key=os.urandom(12).hex() # 設定 Session 的密鑰，才能開始使用!!

# 使用 GET 方法，處理路徑 / 的對應函式
@week4_hw4_jsmethod.route("/", methods=["GET"])
...
@week4_hw4_jsmethod.route("/signin", methods=["POST"])
def signin():
...
# 處理路徑 /member 的對應函式
@week4_hw4_jsmethod.route("/member")
def member():
...
# 處理路徑 /square/ 的對應函式
# 注意動態路由的<"名稱"> 外面的<>只是語法，在實際 url 上不會出現<number>
@week4_hw4_jsmethod.route("/square/<number>")
def square_2(number):
    num=int(number)**2
    return render_template("status.html",header="正整數平方計算結果", login_infor=num)

# 啟動網站伺服器，可透過 port 參數指定埠號
week4_hw4_jsmethod.run(port=3000)